...
import pandas as pd
from mpl_toolkits.mplot3d import Axes3D   # ← tambahan untuk 3D Scatter
...
# menu sidebar  ← ditambahkan menu baru "3D Scatter Plot"
...
def scatter_3_variabel():
    ...
    if jenis_eskrim == 'Cokelat':
        penjualan_selected = df['Penjualan_Coklat']
    elif jenis_eskrim == 'Vanila':
        penjualan_selected = df['Penjualan_Vanila']
    else:
        penjualan_selected = df['Penjualan_Stroberi']

    st.subheader('Data Penjualan dan Suhu')
    st.dataframe(df)

    fig, ax = plt.subplots()
    scatter = ax.scatter(df['Suhu'], penjualan_selected, c=df['Kelembapan'], s=100, cmap='coolwarm', alpha=0.7)
...
def scatter_3d():
    ...
    ax.scatter(suhu, penjualan_weekdays, penjualan_weekend,
               color="orange", s=80, label="Data Penjualan")
